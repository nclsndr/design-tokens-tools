...
describe('Token', () => {
  describe('Getters', () => {
    it('should access the token properties', () => {
      ...
      const redToken = tokenTree.getToken(['color', 'red']).match({
        Some: (token) => token,
        None: () => {
          throw new Error('Token not found');
        },
      ...
      const yellowToken = tokenTree.getToken(['color', 'yellow']).match({
        Some: (token) => token,
        None: () => {
          throw new Error('Token not found');
        },
  ...
  Scalar: #cfd5e6
      ...
      const strokeStyleToken = tokenTree.getToken(['strokeStyle']).match({
        Some: (token) => token,
        None: () => {
          throw new Error('Token not found');
        },
  ...
  ObjectValue: {
    ...
    it('should help iterate over a scalar primitive value', () => {
      ...
      const blueToken = tokenTree
        .getTokenOfType('color', ['color', 'blue'])
        .match({
          Some: (token) => token,
          None: () => {
            throw new Error('Token not found');
